	...
	for _,slot in pairs(slots) do
		...
		if itemLink then
			...
			if slot == "MainHandSlot" or slot == "SecondaryHandSlot" then
				...
				if equiploc == "INVTYPE_2HWEAPON" or equiploc == "INVTYPE_RANGED" or (equiploc == "INVTYPE_RANGEDRIGHT" and subclassid ~= 19) then
			...
			if source then
				...
				for _, entry in pairs(source) do
					if cat.cats[entry] == nil then
						cat.cats[entry] = { catlevel = sourcelevel, count = 0, cats = {}, items = {} }
					end
					cat = cat.cats[entry]
...
function IE:AddItems(tab, padding, extra)
	for _, item in pairs(tab) do
		...
		if InspectEquipConfig.checkEnchants and (item.enchant == nil) and noEnchantWarningSlots[item.slot] and not isArtifactWeapon then
...
function IE.Line_OnEnter(row)
	if row.link then
		...
		if row.item and InspectEquipConfig.checkEnchants and (row.item.enchant == nil) and noEnchantWarningSlots[row.item.slot] and not isArtifactWeapon then
			if WeaponEnchantOnly then
